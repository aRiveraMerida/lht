    ...
    <div>
      ...
      <section className="ed-container" style={{ paddingTop: 130, paddingBottom: 60 }}>
        ...
        >
          ...
          <span aria-hidden="true">/</span>
          <span style={{ opacity: 0.85 }}>{breadcrumbLabel}</span>
        </nav>

        <SectionHeader
          idx="Laboratorio"
          tag={`${lab.stats.guides} ${lab.stats.guides === 1 ? 'capítulo' : 'capítulos'} · ${lab.stats.blocks} ${lab.stats.blocks === 1 ? 'bloque' : 'bloques'}`}
        />
        ...
        >
          <span className="ed-meta">
            <span style={{ color: 'var(--color-don-red)' }}>●</span> {lab.stats.guides} capítulos
          </span>
          <span className="ed-meta opacity-60">{lab.stats.blocks} bloques</span>
          <span className="ed-meta opacity-60">{lab.stats.updated}</span>
        </div>
      </section>
      ...
      {/* Blocks */}
      {lab.blocks.map((block, bi) => {
        const prev = lab.blocks[bi - 1]
        const next = lab.blocks[bi + 1]
        return (
          <section
            key={block.id}
            id={block.id}
